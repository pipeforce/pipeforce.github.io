...
- `requiredVersion` defines the minimum version of the PIPEFORE backend which is required in order to run the apps defined in this workspace. For example `9.0.0` or just `9`. If this value is empty or null, specific version check will be done. This will be checked in case the workspace apps are installed using the command `app.install` or via marketplace.
